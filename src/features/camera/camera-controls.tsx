    ...
    return (
      <div className="h-full bg-gradient-to-l from-black/95 via-black/80 to-transparent pr-[max(1.25rem,env(safe-area-inset-right))] pl-3 py-4 flex flex-row items-center gap-3 z-30 select-none">
        ...
        {zoomCapabilities && zoomPresets.length > 1 && onZoomChange && (
          <div className="flex flex-col items-center gap-1.5 p-1 rounded-full bg-black/60 backdrop-blur-md border border-white/15 shadow-xl">
            ...
            {!zoomPresets.some((level) => Math.abs(zoom - level) < 0.1) && (
              <span className="py-1 px-1 rounded-full bg-[#c5984f] text-[#08111d] text-[10px] font-black shadow-md text-center">
                {zoom.toFixed(1)}×
              </span>
            )}
          </div>
